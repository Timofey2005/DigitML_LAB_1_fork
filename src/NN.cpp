...
// Безопасный логарифм для векторов
static std::vector<double> vec_log(const std::vector<double>& vec) {
    constexpr double eps = 1e-12;
    std::vector<double> result(vec.size());
    for (size_t i = 0; i < vec.size(); ++i) {
        result[i] = std::log(std::max(vec[i], eps));
    }
    return result;
}

// Оператор разности для векторов
std::vector<double> operator-(const std::vector<double>& lhs,
                               const std::vector<double>& rhs) {
    if (lhs.size() != rhs.size()) {
        throw std::runtime_error("vector-: inconsistent size");
    }
...
// Обучение
void NeuralNetwork::train(const unsigned int iterations,
    ...
    for (unsigned int i = 0; i < iterations; ++i) {
        ...
        printf("Cost after %u iteration(s): %f\n", i + 1, cost);
...
std::vector<double> vectorize_label(unsigned char label) {
    ...
    result[static_cast<unsigned int>(label)] = 1.0;
    return result;
}

// Вычисление градиентов и стоимости
void NeuralNetwork::compute_gradients_and_cost(
    ...
    for (unsigned int i = 0; i < m; ++i) {
        ...
        std::vector<double> hidden_layer = feed_forward(first_layer, weights1);
        hidden_layer.insert(hidden_layer.begin(), 1.0);

        std::vector<double> last_layer = feed_forward(hidden_layer, weights2);

        const std::vector<double> vector_outcome = vectorize_label(labels[i][0]);
        const std::vector<double> ones(10, 1.0);

        const double first_part =
            ((Matrix<double>(vector_outcome) * -1.0).transpose() * vec_log(last_layer))[0];
        const double second_part =
            ((Matrix<double>(ones - vector_outcome)).transpose() * vec_log(ones - last_layer))[0];

        cost += (first_part - second_part) / static_cast<double>(m);
        ...
        for (unsigned int j = 0; j < HIDDEN_SIZE; ++j) d2_vec[j] = d2[j + 1][0];
    ...
    for (unsigned int i = 0; i < temp_weights1.rows(); ++i) temp_weights1[i][0] = 0.0;
    Matrix<double> temp_weights2(weights2);
    for (unsigned int i = 0; i < temp_weights2.rows(); ++i) temp_weights2[i][0] = 0.0;

    gradient_1 = gradient_1 / static_cast<double>(m) + temp_weights1 * (lambda / m);
    gradient_2 = gradient_2 / static_cast<double>(m) + temp_weights2 * (lambda / m);
...
Matrix<double> NeuralNetwork::weight_init(double maxWeight,
                                          unsigned int rows,
                                          unsigned int cols) {
...
std::vector<double> NeuralNetwork::bent_identity(const std::vector<double>& x) {
    std::vector<double> result(x.size());
    for (size_t i = 0; i < x.size(); ++i)
        result[i] = (std::sqrt(x[i] * x[i] + 1.0) - 1.0) / 2.0 + x[i];
    return result;
}
...
std::vector<double> NeuralNetwork::sigmoid_prime(const std::vector<double>& x) {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
        const double t = std::exp(x[i]);
        result[i] = t / ((1.0 + t) * (1.0 + t));
    }
    return result;
}
